...
def update(region,country,span_year):
    ...
    geo_df_filter = geo_df[
        (geo_df['Region']==region) & (geo_df['Country']==country) & (geo_df['Year']>=span_year[0]) & (geo_df['Year']<=span_year[1])
    ]

    fig = px.scatter_mapbox(geo_df_filter, lat="latitude", lon="longitude", color="Killed", size="AttackCount",
